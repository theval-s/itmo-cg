...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        if (i%2 == 0) {
            game->Components.push_back(new val_cg::PlanetComponent(game, i+2.f, 1.f, 1.f, dist(gen), {dist(gen), dist(gen), dist(gen), 1}, tmp));
        } else {
            tmp = new val_cg::PlanetComponent(game, i+2.f, 1.f, 1.f, dist(gen), {dist(gen), dist(gen), dist(gen), 1}, Sun);
